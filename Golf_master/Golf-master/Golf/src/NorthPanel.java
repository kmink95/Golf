...
class NorthPanel extends JPanel implements ActionListener, Runnable {
	...
	private JButton broad = new JButton("안내방송♪");
	private JButton end_broad = new JButton("종료방송♪");
	...
	public NorthPanel() {
		...
		end_broad.addActionListener(new ActionListener() {
	...
	public void run() {
		...
		while (true) {
			...
			if(ask_box.isIt == 0 && date.get(Calendar.DAY_OF_WEEK) != 1 && date.get(Calendar.DAY_OF_WEEK) != 7 && date.get(Calendar.HOUR_OF_DAY) < var_setting.weekday_time_limit_shorter_started_time )
			 {
				long_time_button.setBackground(Color.yellow);
			...
			//shorter time
			 ...
			 {
				 short_time_button.setBackground(Color.yellow);
